...
/**
 * @brief Processes a specific quadrant of the image and applies the
 * transformation (rotation + scaling).
 *
 * This function processes a rectangular region (quadrant) of the transformed
 * image based on the provided bounds (start and end rows and columns). It
 * computes the corresponding coordinates in the original image using the
 * inverse transformation matrix, and then maps the pixel values from the
 * original image to the transformed image.
 *
 * The function performs the following steps:
 * 1. For each pixel in the specified quadrant, it calculates the corresponding
 * pixel coordinates in the original image by applying the inverse
 * transformation matrix.
 * 2. It then checks whether the calculated coordinates are within the bounds of
 * the original image.
 * 3. If the coordinates are valid, it copies the pixel values from the original
 * image to the transformed image; otherwise, it sets the pixel value in the
 * transformed image to zero (black).
 *
 * @param transformedImage The image that will store the result of the
 * transformation.
 * @param transformMatrix The transformation matrix (including rotation and
 * scaling).
 ...
 */
void processQuadrant(Image &transformedImage,
                     const Eigen::Matrix2f &transformMatrix,
                     const Eigen::Vector2f &centerOriginal,
                     const Eigen::Vector2f &centerNew, int startRow, int endRow,
                     int startCol, int endCol, int width, int height,
...
#pragma omp parallel for collapse(2)
  for (int rowDiv = 0; rowDiv < numDivisions; rowDiv++) {
    for (int colDiv = 0; colDiv < numDivisions; colDiv++) {
      ...
      int endRow =
          (rowDiv == numDivisions - 1) ? newHeight : (rowDiv + 1) * blockRows;
      int startCol = colDiv * blockCols;
      int endCol =
          (colDiv == numDivisions - 1) ? newWidth : (colDiv + 1) * blockCols;

      auto threadStart = high_resolution_clock::now();

      processQuadrant(transformedImage, transformMatrix, centerOriginal,
                      centerNew, startRow, endRow, startCol, endCol, width,
                      height, channels, data);

      auto threadEnd = high_resolution_clock::now();
      long long threadDuration =
          duration_cast<milliseconds>(threadEnd - threadStart).count();
  ...
  for (size_t i = 0; i < threadTimes.size(); i++) {
    if (threadTimes[i] > 0) {
      cout << "🧵 Hilo " << i + 1 << " tiempo: " << threadTimes[i] << " ms"
           << endl;
    }
  }
