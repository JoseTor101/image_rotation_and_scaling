...
void Image::image(const char *path){
    ...
    if (data) {
        cout << "Imagen cargada: " << width << "x" << height << " con " << channels << " canales.\n";
    } else {
...
void Image::rotateImage(int angle) {
    ...
    for (int i = 0; i < newHeight; i++) {
        for (int j = 0; j < newWidth; j++) {
            ...
            Eigen::Vector2f oldCoords = rotationMatrix.inverse() * (newCoords - centerNew) + centerOriginal;

            int x = round(oldCoords[0]);
            int y = round(oldCoords[1]);

            // Check if the transformed coordinates are inside the original image bounds
            if (x >= 0 && x < width && y >= 0 && y < height) {
                // Copy pixel values from original image
                for (int c = 0; c < channels; c++) {
                    rotatedImage.data[(i * newWidth + j) * channels + c] = data[(y * width + x) * channels + c];
                }
            } else {
